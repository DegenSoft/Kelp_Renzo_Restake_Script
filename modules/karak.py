...
class Karak(BaseModule):
        ...
        if self.config.get("withdraw_start"):
            ...
            self.contract = self.web3.eth.contract(
                self.web3.to_checksum_address(
                    self.DELEGATION_SUPERVISER_CONTRACTS[chain_id]
                ),
                abi=json.loads(self.DELEGATION_SUPERVISER_ABI),
            )
            self.dep_contract = self.web3.eth.contract(
                self.web3.to_checksum_address(self.CONTRACTS[chain_id]),
                abi=json.loads(self.ABI),
            )
            erc20_contract_address = (
                self.KARAK_PUFFETH_CONTRACT_ADDRESS
                if chain_id == 1
                else self.KARAK_EZETH_CONTRACT_ADDRESS
            )
            token = Erc20Token(self.web3, erc20_contract_address)
            symbol = await token.symbol()
            balance_pool = await self.dep_contract.functions.getDeposits(
                account.address
            ).call()
            logger.debug(
                f'{symbol} balance {self.web3.from_wei(balance_pool[3][0], "ether")} {symbol} in Karak pool'
            )
            if not balance_pool:
                logger.error(f"no {symbol} balance")
                return f"no {symbol} balance"
            value = self._get_value(balance_pool[3][0])
            karak_contract_address = (
                self.KARAK_PUFFETH_CONTRACT_ADDRESS
                if chain_id == 1
                else self.KARAK_EZETH_CONTRACT_ADDRESS
            )
            tx_call = self.contract.functions.startWithdraw(
                [([karak_contract_address], [value], account.address)]
            )
            logger.debug(
                f'Karak -> {self.web3.from_wei(value, "ether"):.4f} {symbol} | In queue, come back in 7 days'
            )
            ...
            token = Erc20Token(self.web3, erc20_contract_address)
            symbol = await token.symbol()
            balance_pool = await self.contract.functions.fetchQueuedWithdrawals(
                account.address
            ).call()
            print(balance_pool)
            logger.debug(
                f'{symbol} balance for withdraw {self.web3.from_wei(balance_pool[0][4][1][0], "ether")} {symbol} in Karak pool'
            )
            if not balance_pool:
            ...
            logger.debug(
                f'Karak -> {self.web3.from_wei(balance_pool[0][4][1][0], "ether"):.4f} {symbol} '
            )
            tx = await self.build_transaction(account, tx_call, 0)
            return await self.send_and_wait_for_transaction(account, tx)
        else:
            ...
            erc20_contract_address = (
                self.PUFFER_PUFFETH_CONTRACT_ADDRESS
                if chain_id == 1
            ...
            logger.debug(
                f'{symbol} balance {self.web3.from_wei(balance, "ether")} {symbol}'
            )
            if not balance:
                logger.error(f"no {symbol} balance")
            ...
            tx_call = self.contract.functions.deposit(
                karak_contract_address, value, int(value * 0.99)
            )
    ...
    def _get_value(self, balance):
        ...
        if not self.config["amount_percent"]:
            value = random_float(
                self.config["amount"][0], min(self.config["amount"][1], _max_value)
            )
        else:
            if (
                self.config["amount_percent"][0]
                == self.config["amount_percent"][1]
                == 100
            ):
                return balance
    ...
    async def _login_karak(self, account):
        ...
        response = requests.get(
            "https://restaking-backend.karak.network/getReferrals",
            params=params,
            headers=headers,
            proxies=proxies,
        )
        ...
        response = requests.get(
            "https://restaking-backend.karak.network/verifyReferral",
            params=params,
            headers=headers,
            proxies=proxies,
        )
        ...
        response = requests.post(
            "https://restaking-backend.karak.network/startRegisterWallet",
            params=params,
            headers=headers,
            json=json_data,
            proxies=proxies,
        ...
        if response.status_code != 200:
            raise RuntimeError(
                "could not get message to sign in to karak: %s" % response.text
            )
        res = response.json()
        ...
        response = requests.post(
            "https://restaking-backend.karak.network/finishRegisterWallet",
            params=params,
            headers=headers,
            json=json_data,
            proxies=proxies,
        ...
        if response.status_code != 200:
            raise RuntimeError(
                "error registering wallet: %s" % response.json()[0]["error"]["message"]
            )
